...
class ReflectionPropertiesProvider
{
    ...
    private static function yieldFilteredClassProperties(int ...$problemTypes): ?Generator
    {
        ...
        $toYield = array_filter(array_map(fn($class) => EntitiesFilter::getFiltered($class->properties, fn(PHPProperty $property) => $property->access === 'private' || $class->name === "DOMException", ...$problemTypes), $filteredClasses), fn($array) => !empty($array));
        if (empty($toYield)) {
            yield [null, null];
        }
