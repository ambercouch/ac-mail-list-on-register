...
use ACSB\Vendor\StubTests\TestData\Providers\Reflection\ReflectionConstantsProvider;
...
use ACSB\Vendor\StubTests\TestData\Providers\Reflection\ReflectionPropertiesProvider;
...
class StubsPhp81Tests extends AbstractBaseStubsTestCase
{
    ...
    public function testPropertyReadonly(?string $classId, ?string $propertyName)
    {
        ...
        $reflectionProperty = ReflectionStubsSingleton::getReflectionStubs()->getClass($classId, fromReflection: \true)->getProperty($propertyName, fromReflection: \true);
        $stubProperty = PhpStormStubsSingleton::getPhpStormStubs()->getClass($classId)->getProperty($propertyName);
        static::assertEquals($reflectionProperty->isReadonly, $stubProperty->isReadonly, "Property {$classId}::{$propertyName} readonly modifier is incorrect");
    }
    ...
    public function testClassMethodTentativeReturnTypeHints(?string $classId, ?string $methodName)
    {
        ...
        self::assertTrue($testCondition, "Method {$classId}::{$methodName} has invalid return type. Reflection method has return type " . implode('|', $reflectionMethod->returnTypesFromSignature) . ' but stubs has return type ' . implode('|', $stubsMethod->returnTypesFromSignature) . ' in signature and attribute has types ' . implode('|', $typesFromAttribute));
        self::assertTrue($stubsMethod->isReturnTypeTentative, "Reflection method {$classId}::{$methodName} has " . "tentative return type but stub's method isn't declared as tentative");
    }
    ...
    public function testEnumConstants(?string $classId, ?string $constantName): void
    {
        ...
        $reflectionConstant = ReflectionStubsSingleton::getReflectionStubs()->getEnum($classId, fromReflection: \true)->getConstant($constantName, fromReflection: \true);
        ...
        static::assertNotEmpty($stubConstant, "Missing constant: {$classId}::{$constantName} = {$constantValue}\n");
    }
    ...
    public function testEnumCases(?string $classId, ?string $caseName): void
    {
        ...
        $reflectionConstant = ReflectionStubsSingleton::getReflectionStubs()->getEnum($classId, fromReflection: \true)->getCase($caseName, fromReflection: \true);
        ...
        static::assertNotEmpty($stubConstant, "Missing enum case: {$classId}::{$caseName}\n");
    }
    ...
    public function testEnumMethodsDefaultParametersValue(?string $classId, ?string $methodName, ?string $parameterName)
    {
        ...
        self::assertEquals($reflectionValue, $stubValue, sprintf('Reflection method %s::%s has optional parameter %s with default value %s but stub parameter has value %s', $classId, $methodName, $parameterName, $reflectionValue, $stubValue));
    }
    ...
    public function testEnumMethodsWithoutOptionalDefaultParametersValue(?string $classId, ?string $methodName, ?string $parameterName)
    {
        ...
        $stubParameters = array_filter($phpstormFunction->parameters, fn(PHPParameter $stubParameter) => $stubParameter->indexInSignature === $reflectionParameter->indexInSignature);
        $stubOptionalParameter = array_pop($stubParameters);
        self::assertEmpty($stubOptionalParameter->defaultValue, sprintf('Stub method %s::%s has a parameter "%s" which expected to have no default value but it has', $classId, $methodName, $stubOptionalParameter->name));
        self::assertTrue($stubOptionalParameter->markedOptionalInPhpDoc, sprintf('Stub method %s::%s has a parameter "%s" which expected to be marked as [optional] at PHPDoc but it is not', $classId, $methodName, $stubOptionalParameter->name));
    }
}
