...
use Exception;
use ACSB\Vendor\phpDocumentor\Reflection\DocBlock\Tags\PropertyRead;
...
class PHPClass extends BasePHPClass
{
    ...
    public function readObjectFromStubNode($node)
    {
        ...
        if ($node->getDocComment() !== null) {
            ...
            $properties = array_merge($docBlock->getTagsByName('property-read'), $docBlock->getTagsByName('property'));
            foreach ($properties as $property) {
                ...
                assert(!array_key_exists($propertyName, $this->properties), "Property '{$propertyName}' is already declared in class '{$this->name}'");
    ...
    public function addProperty(PHPProperty $parsedProperty)
    {
        if (isset($parsedProperty->name)) {
            if (array_key_exists($parsedProperty->name, $this->properties)) {
                $amount = count(array_filter($this->properties, function (PHPProperty $nextProperty) use ($parsedProperty) {
    ...
    public function getProperty($propertyName, $fromReflection = \false)
    {
        if ($fromReflection) {
            ...
            $properties = array_filter($this->properties, function (PHPProperty $property) use ($propertyName) {
                return $property->name === $propertyName && $property->duplicateOtherElement === \false && ParserUtils::entitySuitsCurrentPhpVersion($property);
            });
        }
        if (empty($properties)) {
