 ...
 */
namespace ACSB\Vendor\Humbug\PhpScoper\Autoload;
...
final readonly class ComposerFileHasher
{
    ...
    public static function create(string $vendorDir, string $rootDir, array $filePaths): self
    {
        $vendorDirRelativeToRoot = Path::makeRelative($vendorDir, $rootDir);
        $packagePathRegex = sprintf(self::PACKAGE_PATH_REGEX, $vendorDirRelativeToRoot);
        return new self($rootDir, $filePaths, $packagePathRegex);
    }
    public function __construct(private string $rootDir, private array $filePaths, private string $packagePathRegex)
    {
    }
    /**
     * @return string[]
    ...
    {
        return array_map($this->generateHash(...), $this->filePaths);
    }
    /**
