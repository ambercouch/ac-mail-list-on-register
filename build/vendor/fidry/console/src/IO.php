 ...
 */
namespace ACSB\Vendor\Fidry\Console;

use Closure;
use ACSB\Vendor\Fidry\Console\Input\DecoratesInput;
use ACSB\Vendor\Fidry\Console\Input\TypedInput;
use ACSB\Vendor\Fidry\Console\Output\DecoratesLogger;
use ACSB\Vendor\Fidry\Console\Output\DecoratesOutput;
use ACSB\Vendor\Fidry\Console\Output\DecoratesStyledOutput;
use ACSB\Vendor\Fidry\Console\Output\StyledOutput;
use ACSB\Vendor\Fidry\Console\Output\SymfonyStyledOutput;
use ACSB\Vendor\Psr\Log\LoggerInterface;
use ACSB\Vendor\Symfony\Component\Console\Formatter\OutputFormatterInterface;
use ACSB\Vendor\Symfony\Component\Console\Helper\ProgressBar;
use ACSB\Vendor\Symfony\Component\Console\Helper\Table;
use ACSB\Vendor\Symfony\Component\Console\Helper\TableSeparator;
...
use ACSB\Vendor\Symfony\Component\Console\Input\StringInput;
use ACSB\Vendor\Symfony\Component\Console\Logger\ConsoleLogger;
...
use ACSB\Vendor\Symfony\Component\Console\Output\NullOutput;
use ACSB\Vendor\Symfony\Component\Console\Output\OutputInterface;
use ACSB\Vendor\Symfony\Component\Console\Question\Question;
/**
...
final class IO implements InputInterface, OutputInterface, StyledOutput
{
     ...
     */
    public function __construct(private readonly InputInterface $input, private readonly OutputInterface $output, ?Closure $styledOutputFactory = null, ?Closure $loggerFactory = null)
    {
    ...
    {
        return new self($this->input, $this->getErrorOutput(), $this->styledOutputFactory, $this->loggerFactory);
    }
    public function withInput(InputInterface $input): self
    {
        return new self($input, $this->output, $this->styledOutputFactory, $this->loggerFactory);
    }
    ...
    {
        return new self($this->input, $output, $this->styledOutputFactory, $this->loggerFactory);
    }
    ...
    {
        return new self($this->input, $this->output, $styledOutputFactory, $this->loggerFactory);
    }
    ...
    {
        return new self(new StringInput(''), $this->output, $this->styledOutputFactory, $loggerFactory);
    }
    ...
    {
        Deprecation::trigger('::getArgument() will be reverted to return the traditional Symfony argument raw value. Use ::getTypedArgument() instead.', '0.6.0');
