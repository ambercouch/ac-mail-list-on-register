...
use ACSB\Vendor\phpDocumentor\Reflection\Type;
use ACSB\Vendor\PhpParser\Node;
use ACSB\Vendor\PhpParser\Node\AttributeGroup;
...
abstract class BasePHPElement
{
    ...
    protected static function getReflectionTypeAsArray($type)
    {
        ...
        if ($type instanceof ReflectionNamedType) {
            $type->allowsNull() && $type->getName() !== 'mixed' ? array_push($reflectionTypes, '?' . $type->getName()) : array_push($reflectionTypes, $type->getName());
        }
            ...
            foreach ($attrGroup->attrs as $attr) {
                if ($attr->name->toString() === LanguageLevelTypeAware::class) {
                    ...
                    foreach ($versionTypesMap as $item) {
                        $types[number_format((float) $item->key->value, 1)] = explode('|', preg_replace('/\w+\[]/', 'array', $item->value->value));
                    }
                ...
                if ($attr->name->toString() === PhpStormStubsElementAvailable::class) {
                    if (count($attr->args) === 2) {
                        ...
                        if ($arg instanceof Array_) {
                            ...
                            return $rangeName === null || $rangeName->name === 'from' ? ['from' => (float) $arg->value, 'to' => PhpVersions::getLatest()] : ['from' => PhpVersions::getFirst(), 'to' => (float) $arg->value];
                        }
                    }
                }
            }
    ...
    public function hasMutedProblem($stubProblemType)
    {
        if (array_key_exists($stubProblemType, $this->mutedProblems)) {
            if (in_array('ALL', $this->mutedProblems[$stubProblemType], \true) || in_array((float) getenv('PHP_VERSION'), $this->mutedProblems[$stubProblemType], \true)) {
                return \true;
            }
        }
        return \false;
    ...
    {
        $this->isDeprecated = self::hasDeprecatedAttribute($node) && self::deprecatedVersionSuitsCurrentLanguageLevel($node) || !empty($this->deprecatedTags) && self::deprecatedVersionSuitsCurrentLanguageLevel();
    }
