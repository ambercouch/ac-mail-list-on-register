...
use ACSB\Vendor\Humbug\PhpScoper\PhpParser\Node\NameFactory;
use ACSB\Vendor\PhpParser\Node;
use ACSB\Vendor\PhpParser\Node\Stmt;
use ACSB\Vendor\PhpParser\Node\Stmt\Declare_;
use ACSB\Vendor\PhpParser\Node\Stmt\GroupUse;
use ACSB\Vendor\PhpParser\Node\Stmt\InlineHTML;
...
use ACSB\Vendor\PhpParser\NodeTraverserInterface;
use ACSB\Vendor\PhpParser\NodeVisitor;
...
final readonly class NodeTraverser implements NodeTraverserInterface
{
    ...
    {
        return array_map(static fn(UseItem $use): Use_ => self::createUseNode($use, $node), $node->uses);
    }
    private static function createUseNode(UseItem $use, GroupUse $groupUse): Use_
    {
        $newUseItem = self::prefixName($use, $groupUse);
        return new Use_([$newUseItem], $groupUse->type, $groupUse->getAttributes());
    }
    private static function prefixName(UseItem $use, GroupUse $groupUse): UseItem
    {
        $prefixedName = NameFactory::concat($groupUse->prefix, $use->name);
        return new UseItem($prefixedName, $use->alias, $use->type, $use->getAttributes());
    }
}
