 ...
 */
namespace ACSB\Vendor\Humbug\PhpScoper\Configuration;

use Exception;
use ACSB\Vendor\Humbug\PhpScoper\Configuration\Throwable\InvalidConfiguration;
use ACSB\Vendor\Humbug\PhpScoper\Configuration\Throwable\InvalidConfigurationFile;
use ACSB\Vendor\Humbug\PhpScoper\Configuration\Throwable\InvalidConfigurationValue;
use ACSB\Vendor\Humbug\PhpScoper\Patcher\ComposerPatcher;
use ACSB\Vendor\Humbug\PhpScoper\Patcher\Patcher;
use ACSB\Vendor\Humbug\PhpScoper\Patcher\PatcherChain;
use ACSB\Vendor\Humbug\PhpScoper\Patcher\SymfonyParentTraitPatcher;
use ACSB\Vendor\Humbug\PhpScoper\Patcher\SymfonyPatcher;
...
use function ACSB\Vendor\Humbug\PhpScoper\chain;
...
use function ACSB\Vendor\Safe\file_get_contents;
use function trim;
use const DIRECTORY_SEPARATOR;
final readonly class ConfigurationFactory
{
    public const DEFAULT_FILE_NAME = 'scoper.inc.php';
    public function __construct(private Filesystem $fileSystem, private SymbolsConfigurationFactory $symbolsConfigurationFactory)
    {
    }
    /**
    ...
    public function create(?string $path = null, array $paths = []): Configuration
    {
        ...
        return new Configuration($path, $outputDir, $prefix, self::retrievePhpVersion($config), $filesWithContents, self::retrieveFilesWithContents($excludedFiles), new PatcherChain($patchers), $symbolsConfiguration);
    }
    /**
     * @param string[] $paths
     */
    ...
    {
        array_map(ConfigurationKeys::assertIsValidKey(...), array_keys($config));
    }
    /**
    ...
    private static function retrieveFilesFromPaths(array $paths): iterable
    {
        ...
        $finder = new Finder();
        $finder->files()->in($pathsToSearch)->append($filesToAppend)->filter(static fn(SplFileInfo $fileInfo) => !$fileInfo->isLink())->sortByName();
        return $finder;
    }
    /**
