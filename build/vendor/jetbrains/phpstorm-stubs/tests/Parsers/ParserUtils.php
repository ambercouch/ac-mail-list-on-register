...
use ACSB\Vendor\StubTests\Model\PHPClassConstant;
...
class ParserUtils
{
    ...
    {
        return (bool) preg_match('/^[1-9]+\.\d+(\.[1-9]+\d*)*$/', $tag->getVersion());
        //find version like any but 7.4.0
    }
    ...
    public static function getAvailableInVersions(?BasePHPElement $element): array
    {
        if ($element !== null) {
            ...
            return array_filter(iterator_to_array(new PhpVersions()), fn($version) => $version >= $firstSinceVersion && $version <= $lastAvailableVersion);
        }
        return [];
    }
    /**
    ...
    private static function getLatestAvailableVersionFromPhpDoc(BasePHPElement $element): array
    {
        ...
        if (!empty($element->removedTags) && $element->stubBelongsToCore) {
            ...
            $indexOfRemovedVersion = array_search($removedVersion, iterator_to_array($allVersions), \true);
    ...
    {
        $parentClass = PhpStormStubsSingleton::getPhpStormStubs()->getEnum($element->parentId, sourceFilePath: $element->sourceFilePath, shouldSuitCurrentPhpVersion: \false);
        if ($parentClass === null) {
    ...
    {
        return in_array((float) getenv('PHP_VERSION'), ParserUtils::getAvailableInVersions($element), \true);
    }
}
