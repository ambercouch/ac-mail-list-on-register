...
use ACSB\Vendor\phpDocumentor\Reflection\DocBlock\Tags\Deprecated;
use ACSB\Vendor\phpDocumentor\Reflection\DocBlock\Tags\Link;
use ACSB\Vendor\phpDocumentor\Reflection\DocBlock\Tags\Reference\Url;
use ACSB\Vendor\phpDocumentor\Reflection\DocBlock\Tags\See;
use ACSB\Vendor\phpDocumentor\Reflection\DocBlock\Tags\Since;
use ACSB\Vendor\PHPUnit\Framework\Attributes\DataProviderExternal;
use ACSB\Vendor\StubTests\Model\BasePHPElement;
use ACSB\Vendor\StubTests\Model\PHPDocElement;
...
use ACSB\Vendor\StubTests\TestData\Providers\Stubs\StubConstantsProvider;
...
class StubsPhpDocTest extends AbstractBaseStubsTestCase
{
    ...
    private static function checkHtmlTags(BasePHPElement $element, string $elementName): void
    {
        ...
        $phpdoc = preg_replace(['#<br\s*/>#', '#<br>#i', '#->#', '#=>#', '#"->"#', '# >= #', '#\(>=#', '#\'>\'#', '# > #', '#\?>#', '#`<.*>`#U', '#`.*<.*>`#U', '#<pre>.*</pre>#sU', '#<code>.*</code>#sU', '#@author.*<.*>#U', '#(\s[\w]+[-][\w]+<[a-zA-Z,\s]+>[\s|]+)|([\w]+<[a-zA-Z,|\s]+>[\s|\W]+)#'], '', $phpdoc);
        $countTags = substr_count($phpdoc, '>');
        self::assertSame(0, $countTags % 2, "In {$elementName} phpdoc has a html error and the phpdoc maybe not displayed correctly in PhpStorm: " . print_r($phpdoc, \true));
    }
    private static function checkLinks(BasePHPElement $element, string $elementName): void
    {
        ...
        foreach ($element->links as $link) {
            if ($link instanceof Link) {
                self::assertStringStartsWith('https', $link->getLink(), "In {$elementName} @link doesn't start with https");
    ...
    private static function checkContainsOnlyValidTags(BasePHPElement $element, string $elementName): void
    {
        $VALID_TAGS = [
            ...
            'template',
            'template-implements',
            // https://github.com/JetBrains/phpstorm-stubs/pull/1212#issuecomment-907263735
            'template-extends',
            'template-covariant',
            'todo',
            'uses',
