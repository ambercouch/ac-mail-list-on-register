 ...
 */
namespace ACSB\Vendor\Symfony\Component\VarDumper\Caster;

use ACSB\Vendor\Symfony\Component\VarDumper\Cloner\Stub;
/**
...
class DOMCaster
{
    ...
    public static function castException(\DOMException|\ACSB\Vendor\Dom\Exception $e, array $a, Stub $stub, bool $isNested): array
    {
    ...
    {
        $a += ['doctype' => $dom->doctype, 'implementation' => $dom->implementation, 'documentElement' => new CutStub($dom->documentElement), 'inputEncoding' => $dom->inputEncoding, 'xmlEncoding' => $dom->xmlEncoding, 'xmlStandalone' => $dom->xmlStandalone, 'xmlVersion' => $dom->xmlVersion, 'documentURI' => $dom->documentURI ? new LinkStub($dom->documentURI) : $dom->documentURI, 'formatOutput' => $dom->formatOutput];
        if (!($filter & Caster::EXCLUDE_VERBOSE)) {
            $formatOutput = $dom->formatOutput;
            $dom->formatOutput = \true;
            $a += [Caster::PREFIX_VIRTUAL . 'xml' => $dom->saveXML()];
    ...
    {
        $a += ['doctype' => $dom->doctype, 'implementation' => $dom->implementation, 'documentElement' => new CutStub($dom->documentElement), 'inputEncoding' => $dom->inputEncoding, 'documentURI' => $dom->documentURI ? new LinkStub($dom->documentURI) : $dom->documentURI];
        if (!($filter & Caster::EXCLUDE_VERBOSE)) {
    ...
    {
        $a += ['name' => $dom->name, 'specified' => $dom->specified, 'value' => $dom->value, 'ownerElement' => $dom->ownerElement];
        if ($dom instanceof \DOMAttr) {
            $a += ['schemaTypeInfo' => $dom->schemaTypeInfo];
        }
        return $a;
    }
