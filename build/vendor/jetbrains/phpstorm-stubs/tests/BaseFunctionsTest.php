...
class BaseFunctionsTest extends AbstractBaseStubsTestCase
{
    ...
    public function testFunctionsExist(?string $functionId): void
    {
        ...
        $reflectionFunction = ReflectionStubsSingleton::getReflectionStubs()->getFunction($functionId, fromReflection: \true);
    ...
    public function testFunctionsDeprecation(?string $functionId)
    {
        ...
        $reflectionFunction = ReflectionStubsSingleton::getReflectionStubs()->getFunction($functionId, fromReflection: \true);
        $stubFunction = PhpStormStubsSingleton::getPhpStormStubs()->getFunction($functionId);
        static::assertEquals($reflectionFunction->isDeprecated, $stubFunction->isDeprecated, "Deprecation of function {$functionId} is incorrect");
    }
    ...
    public function testFunctionsParametersAmount(?string $functionId)
    {
        ...
        $reflectionFunction = ReflectionStubsSingleton::getReflectionStubs()->getFunction($functionId, fromReflection: \true);
        $stubFunction = PhpStormStubsSingleton::getPhpStormStubs()->getFunction($functionId);
        $filteredStubParameters = array_filter($stubFunction->parameters, fn($parameter) => ParserUtils::entitySuitsCurrentPhpVersion($parameter));
        $uniqueParameterNames = array_unique(array_map(fn(PHPParameter $parameter) => $parameter->name, $filteredStubParameters));
        static::assertSameSize($reflectionFunction->parameters, $uniqueParameterNames, "Parameter number mismatch for function {$functionId}. \n                Expected: " . AbstractBaseStubsTestCase::getParameterRepresentation($reflectionFunction) . "\n" . 'Actual: ' . AbstractBaseStubsTestCase::getParameterRepresentation($stubFunction));
    }
    public function testFunctionsDuplicates()
    {
        $filtered = EntitiesFilter::getFiltered(PhpStormStubsSingleton::getPhpStormStubs()->getFunctions(), problemTypes: StubProblemType::HAS_DUPLICATION);
        $duplicates = self::getDuplicatedFunctions($filtered);
        self::assertCount(0, $duplicates, "Functions \"" . implode(', ', $duplicates) . "\" have duplicates in stubs.\nPlease use #[LanguageLevelTypeAware] or #[PhpStormStubsElementAvailable] if possible");
    }
    ...
    public function testFunctionsOptionalParameters(?string $functionId, ?string $parameterName)
    {
        ...
        self::assertEquals($relfectionParameter->isOptional, $stubOptionalParameter->isOptional, sprintf('Reflection function %s %s optional parameter %s with index %d 
            but stubs parameter %s with index %d %s', $reflectionFunction->name, $relfectionParameter->isOptional ? 'has' : 'has no', $relfectionParameter->name, $relfectionParameter->indexInSignature, $stubOptionalParameter->name, $stubOptionalParameter->indexInSignature, $stubOptionalParameter->isOptional ? 'is optional' : 'is not optional'));
        self::assertEquals($relfectionParameter->is_vararg, $stubOptionalParameter->is_vararg, sprintf('Reflection function %s %s vararg parameter %s with index %d 
            but stubs parameter %s with index %d %s', $reflectionFunction->name, $relfectionParameter->is_vararg ? 'has' : 'has no', $relfectionParameter->name, $relfectionParameter->indexInSignature, $stubOptionalParameter->name, $stubOptionalParameter->indexInSignature, $stubOptionalParameter->is_vararg ? 'is vararg' : 'is not vararg'));
    }
    ...
    public function testClassMethodsOptionalParameters(?string $classId, ?string $methodName, ?string $parameterName)
    {
        ...
        /** @var PHPParameter $stubOptionalParameter */
        $stubOptionalParameter = array_pop($stubParameters);
        self::assertEquals($reflectionParameter->isOptional, $stubOptionalParameter->isOptional, sprintf('Reflection method %s::%s has %s optional parameter %s with index %d but stub parameter %s with index %d is %s optional', $classId, $methodName, $reflectionParameter->isOptional ? "" : "not", $reflectionParameter->name, $reflectionParameter->indexInSignature, $stubOptionalParameter->name, $stubOptionalParameter->indexInSignature, $stubOptionalParameter->isOptional ? "" : "not"));
    }
    ...
    {
        $implodeFunctions = array_filter(PhpStormStubsSingleton::getPhpStormStubs()->getFunctions(), fn(PHPFunction $function) => $function->id === '\implode');
