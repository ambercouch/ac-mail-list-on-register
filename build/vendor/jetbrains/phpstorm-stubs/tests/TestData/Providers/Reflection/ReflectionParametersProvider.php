...
namespace ACSB\Vendor\StubTests\TestData\Providers\Reflection;
...
class ReflectionParametersProvider
{
    ...
    public static function functionParametersWithTypeProvider(): ?Generator
    {
        $filteredFunctions = EntitiesFilter::getFilteredReflectionFunctions();
        $array = array_map(function (PHPFunction $function) {
            return EntitiesFilter::getFilteredParameters($function, null, StubProblemType::PARAMETER_TYPE_MISMATCH);
        }, $filteredFunctions);
        if (empty($array)) {
            yield [null, null];
        } else {
            foreach ($filteredFunctions as $function) {
                foreach (EntitiesFilter::getFilteredParameters($function, null, StubProblemType::PARAMETER_TYPE_MISMATCH) as $parameter) {
                    yield "{$function->id}({$parameter->name})" => [$function->id, $parameter->name];
                }
            }
        }
    ...
    {
        $filteredFunctions = EntitiesFilter::getFiltered(EntitiesFilter::getFilteredReflectionFunctions(), problemTypes: StubProblemType::FUNCTION_PARAMETER_MISMATCH);
        $array = array_map(function (PHPFunction $function) {
            return EntitiesFilter::getFilteredParameters($function, fn(PHPParameter $parameter) => !$parameter->isOptional, StubProblemType::PARAMETER_TYPE_MISMATCH, StubProblemType::WRONG_OPTIONALLITY);
        }, $filteredFunctions);
        if (empty($array)) {
            yield [null, null];
        } else {
            foreach ($filteredFunctions as $function) {
                foreach (EntitiesFilter::getFilteredParameters($function, fn(PHPParameter $parameter) => !$parameter->isOptional, StubProblemType::PARAMETER_TYPE_MISMATCH, StubProblemType::WRONG_OPTIONALLITY) as $parameter) {
                    yield "{$function->id}({$parameter->name})" => [$function->id, $parameter->name];
                }
            }
        }
    ...
    public static function functionOptionalParametersWithDefaultValueProvider(): ?Generator
    {
        $filteredFunctions = EntitiesFilter::getFilteredReflectionFunctions();
        $array = array_map(function (PHPFunction $function) {
            return EntitiesFilter::getFilteredParameters($function, fn(PHPParameter $parameter) => !$parameter->isOptional, StubProblemType::WRONG_PARAMETER_DEFAULT_VALUE);
        }, $filteredFunctions);
        if (empty($array)) {
            yield [null, null];
        } else {
            foreach ($filteredFunctions as $function) {
                foreach (EntitiesFilter::getFilteredParameters($function, fn(PHPParameter $parameter) => !$parameter->isOptional, StubProblemType::WRONG_PARAMETER_DEFAULT_VALUE) as $parameter) {
                    yield "{$function->id}({$parameter->name})" => [$function->id, $parameter->name];
                }
            }
        }
    ...
    public static function functionOptionalParametersWithoutDefaultValueProvider(): ?Generator
    {
        $filteredFunctions = EntitiesFilter::getFilteredReflectionFunctions();
        $array = array_map(function (PHPFunction $function) {
            return EntitiesFilter::getFilteredParameters($function, fn(PHPParameter $parameter) => !$parameter->isOptional || $parameter->isDefaultValueAvailable || $parameter->is_vararg, StubProblemType::WRONG_PARAMETER_DEFAULT_VALUE);
        }, $filteredFunctions);
        if (empty($array)) {
            yield [null, null];
        } else {
            foreach ($filteredFunctions as $function) {
                foreach (EntitiesFilter::getFilteredParameters($function, fn(PHPParameter $parameter) => !$parameter->isOptional || $parameter->isDefaultValueAvailable || $parameter->is_vararg, StubProblemType::WRONG_PARAMETER_DEFAULT_VALUE) as $parameter) {
                    yield "{$function->id}({$parameter->name})" => [$function->id, $parameter->name];
                }
            }
        }
    ...
    public static function classMethodParametersWithTypeHintProvider(): ?Generator
    {
        ...
        $array = array_filter(array_map(function ($class) {
            return array_filter(array_map(function (PHPMethod $method) {
                return EntitiesFilter::getFilteredParameters($method, null, StubProblemType::PARAMETER_TYPE_MISMATCH);
            }, EntitiesFilter::getFilteredReflectionMethods($class)), fn($arr) => !empty($arr));
        }, $filtered), fn($arr) => !empty($arr));
        if (empty($array)) {
            yield [null, null, null];
            ...
            foreach ($filtered as $class) {
                ...
                if (strncmp($class->name, 'PHP', 3) !== 0) {
                    foreach (EntitiesFilter::getFilteredReflectionMethods($class) as $method) {
                        foreach (EntitiesFilter::getFilteredParameters($method, null, StubProblemType::PARAMETER_TYPE_MISMATCH) as $parameter) {
    ...
    public static function classMethodOptionalParametersProvider(): ?Generator
    {
        ...
        $array = array_filter(array_map(function ($class) {
            return array_filter(array_map(function (PHPMethod $method) {
                return EntitiesFilter::getFilteredParameters($method, null, StubProblemType::WRONG_OPTIONALLITY);
            }, EntitiesFilter::getFilteredReflectionMethods($class)), fn($arr) => !empty($arr));
        }, $filtered), fn($arr) => !empty($arr));
        if (empty($array)) {
            yield [null, null, null];
            ...
            foreach ($filtered as $class) {
                ...
                if (strncmp($class->name, 'PHP', 3) !== 0) {
                    foreach (EntitiesFilter::getFilteredReflectionMethods($class) as $method) {
                        foreach (EntitiesFilter::getFilteredParameters($method, null, StubProblemType::WRONG_OPTIONALLITY) as $parameter) {
                            yield "{$class->id}::{$method->name}({$parameter->name})" => [$class->id, $method->name, $parameter->name];
                        }
                    }
                }
    ...
    public static function classMethodOptionalParametersWithDefaultValueProvider(): ?Generator
    {
        ...
        $array = array_filter(array_map(function ($enum) {
            return array_filter(array_map(function (PHPMethod $method) {
                return EntitiesFilter::getFilteredParameters($method, fn(PHPParameter $parameter) => !$parameter->isOptional || !$parameter->isDefaultValueAvailable, StubProblemType::WRONG_PARAMETER_DEFAULT_VALUE);
            }, EntitiesFilter::getFilteredReflectionMethods($enum)), fn($arr) => !empty($arr));
        }, $filtered), fn($arr) => !empty($arr));
        if (empty($array)) {
            yield [null, null, null];
            ...
            foreach ($filtered as $class) {
                ...
                if (strncmp($class->name, 'PHP', 3) !== 0) {
                    foreach (EntitiesFilter::getFilteredReflectionMethods($class) as $method) {
                        foreach (EntitiesFilter::getFilteredParameters($method, fn(PHPParameter $parameter) => !$parameter->isOptional || !$parameter->isDefaultValueAvailable, StubProblemType::WRONG_PARAMETER_DEFAULT_VALUE) as $parameter) {
                            yield "{$class->id}::{$method->name}({$parameter->name})" => [$class->id, $method->name, $parameter->name];
                        }
                    }
                }
    ...
    public static function classMethodOptionalParametersWithoutDefaultValueProvider(): ?Generator
    {
        ...
        $array = array_filter(array_map(function ($class) {
            return array_filter(array_map(function (PHPMethod $method) {
                return EntitiesFilter::getFilteredParameters($method, fn(PHPParameter $parameter) => !$parameter->isOptional || $parameter->isDefaultValueAvailable || $parameter->is_vararg, StubProblemType::WRONG_PARAMETER_DEFAULT_VALUE);
            }, EntitiesFilter::getFilteredReflectionMethods($class)), fn($arr) => !empty($arr));
        }, $filtered), fn($arr) => !empty($arr));
        if (empty($array)) {
            yield [null, null, null];
            ...
            foreach ($filtered as $class) {
                ...
                if (strncmp($class->name, 'PHP', 3) !== 0) {
                    foreach (EntitiesFilter::getFilteredReflectionMethods($class) as $method) {
                        foreach (EntitiesFilter::getFilteredParameters($method, fn(PHPParameter $parameter) => !$parameter->isOptional || $parameter->isDefaultValueAvailable || $parameter->is_vararg, StubProblemType::WRONG_PARAMETER_DEFAULT_VALUE) as $parameter) {
                            yield "{$class->id}::{$method->name}({$parameter->name})" => [$class->id, $method->name, $parameter->name];
                        }
                    }
                }
