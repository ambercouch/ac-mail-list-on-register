...
namespace ACSB\Vendor\StubTests\TestData\Providers\Stubs;
...
use ACSB\Vendor\StubTests\Parsers\ParserUtils;
use ACSB\Vendor\StubTests\TestData\Providers\EntitiesFilter;
...
class StubMethodsProvider
{
    ...
    public static function allFunctionWithReturnTypeHintsProvider(): ?Generator
    {
        ...
        $filteredFunctions = EntitiesFilter::getFiltered($allFunctions, fn(PHPFunction $function) => empty($function->returnTypesFromSignature) || empty($function->returnTypesFromPhpDoc) || $function->hasTentativeReturnType || in_array('mixed', $function->returnTypesFromSignature), StubProblemType::TYPE_IN_PHPDOC_DIFFERS_FROM_SIGNATURE);
    ...
    public static function allClassesMethodsWithReturnTypeHintsProvider(): ?Generator
    {
        ...
        foreach (EntitiesFilter::getFiltered($coreClasses) as $class) {
            $filteredMethods = EntitiesFilter::getFiltered($class->methods, fn(PHPMethod $method) => empty($method->returnTypesFromSignature) || empty($method->returnTypesFromPhpDoc) || $method->parentId === '\___PHPSTORM_HELPERS\object' || $method->hasTentativeReturnType || in_array('mixed', $method->returnTypesFromSignature), StubProblemType::TYPE_IN_PHPDOC_DIFFERS_FROM_SIGNATURE);
    ...
    public static function allInterfacesMethodsWithReturnTypeHintsProvider(): ?Generator
    {
        ...
        foreach (EntitiesFilter::getFiltered($coreInterfaces) as $class) {
            $filteredMethods = EntitiesFilter::getFiltered($class->methods, fn(PHPMethod $method) => empty($method->returnTypesFromSignature) || empty($method->returnTypesFromPhpDoc) || $method->hasTentativeReturnType || in_array('mixed', $method->returnTypesFromSignature), StubProblemType::TYPE_IN_PHPDOC_DIFFERS_FROM_SIGNATURE);
            foreach ($filteredMethods as $methodName => $method) {
                yield "method {$class->id}::{$methodName}" => [$class->id, $method->name];
            }
        }
    ...
    public static function allEnumsMethodsWithReturnTypeHintsProvider(): ?Generator
    {
        ...
        $array = array_filter(array_map(function (PHPEnum $enum) {
            return EntitiesFilter::getFiltered($enum->methods, fn(PHPMethod $method) => empty($method->returnTypesFromSignature) || empty($method->returnTypesFromPhpDoc) || $method->hasTentativeReturnType || in_array('mixed', $method->returnTypesFromSignature), StubProblemType::TYPE_IN_PHPDOC_DIFFERS_FROM_SIGNATURE);
    ...
    public static function classMethodsForReturnTypeHintTestsProvider(): ?Generator
    {
        ...
        return self::yieldFilteredMethods(PHPClass::class, $filterFunction, StubProblemType::FUNCTION_HAS_RETURN_TYPEHINT, StubProblemType::WRONG_RETURN_TYPEHINT);
    }
    public static function interfaceMethodsForReturnTypeHintTestsProvider(): ?Generator
    {
        ...
        return self::yieldFilteredMethods(PHPInterface::class, $filterFunction, StubProblemType::FUNCTION_HAS_RETURN_TYPEHINT, StubProblemType::WRONG_RETURN_TYPEHINT);
    }
    public static function enumMethodsForReturnTypeHintTestsProvider(): ?Generator
    {
        ...
        return self::yieldFilteredMethods(PHPEnum::class, $filterFunction, StubProblemType::FUNCTION_HAS_RETURN_TYPEHINT, StubProblemType::WRONG_RETURN_TYPEHINT);
    }
    public static function classMethodsForNullableReturnTypeHintTestsProvider(): ?Generator
    {
        ...
        return self::yieldFilteredMethods(PHPClass::class, $filterFunction, StubProblemType::HAS_NULLABLE_TYPEHINT, StubProblemType::WRONG_RETURN_TYPEHINT);
    }
    public static function interfaceMethodsForNullableReturnTypeHintTestsProvider(): ?Generator
    {
        ...
        return self::yieldFilteredMethods(PHPInterface::class, $filterFunction, StubProblemType::HAS_NULLABLE_TYPEHINT, StubProblemType::WRONG_RETURN_TYPEHINT);
    }
    public static function enumMethodsForNullableReturnTypeHintTestsProvider(): ?Generator
    {
        ...
        return self::yieldFilteredMethods(PHPEnum::class, $filterFunction, StubProblemType::HAS_NULLABLE_TYPEHINT, StubProblemType::WRONG_RETURN_TYPEHINT);
    }
    public static function classMethodsForUnionReturnTypeHintTestsProvider(): ?Generator
    {
        ...
        return self::yieldFilteredMethods(PHPClass::class, $filterFunction, StubProblemType::HAS_UNION_TYPEHINT, StubProblemType::WRONG_RETURN_TYPEHINT);
    }
    public static function interfaceMethodsForUnionReturnTypeHintTestsProvider(): ?Generator
    {
        ...
        return self::yieldFilteredMethods(PHPInterface::class, $filterFunction, StubProblemType::HAS_UNION_TYPEHINT, StubProblemType::WRONG_RETURN_TYPEHINT);
    }
    public static function enumMethodsForUnionReturnTypeHintTestsProvider(): ?Generator
    {
        ...
        return self::yieldFilteredMethods(PHPEnum::class, $filterFunction, StubProblemType::HAS_UNION_TYPEHINT, StubProblemType::WRONG_RETURN_TYPEHINT);
    }
    private static function getFilterFunctionForLanguageLevel(string $classType, float $languageVersion): callable
    {
        return match ($classType) {
            PHPClass::class => fn(PHPClass $class, PHPMethod $method, ?float $firstSinceVersion) => !$method->isFinal && !$class->isFinal && $firstSinceVersion !== null && $firstSinceVersion < $languageVersion && !$method->isReturnTypeTentative,
            PHPInterface::class => fn(PHPInterface $class, PHPMethod $method, ?float $firstSinceVersion) => !$method->isFinal && !$class->isFinal && $firstSinceVersion !== null && $firstSinceVersion < $languageVersion && !$method->isReturnTypeTentative,
            PHPEnum::class => fn(PHPEnum $class, PHPMethod $method, ?float $firstSinceVersion) => !$method->isFinal && !$class->isFinal && $firstSinceVersion !== null && $firstSinceVersion < $languageVersion && !$method->isReturnTypeTentative,
    ...
    private static function yieldFilteredMethods(string $classType, callable $filterFunction, int ...$problemTypes): ?Generator
    {
        ...
        $array = array_filter(array_map(function ($class) use ($problemTypes, $filterFunction) {
            return array_filter(EntitiesFilter::getFiltered($class->methods, fn(PHPMethod $method) => $method->parentId === '\___PHPSTORM_HELPERS\object', ...$problemTypes), function ($method) use ($filterFunction, $class) {
                $firstSinceVersion = ParserUtils::getDeclaredSinceVersion($method);
                return $filterFunction($class, $method, $firstSinceVersion) === \true;
            });
