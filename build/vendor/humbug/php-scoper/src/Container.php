 ...
 */
namespace ACSB\Vendor\Humbug\PhpScoper;

use ACSB\Vendor\Humbug\PhpScoper\Configuration\ConfigurationFactory;
use ACSB\Vendor\Humbug\PhpScoper\Configuration\RegexChecker;
use ACSB\Vendor\Humbug\PhpScoper\Configuration\SymbolsConfigurationFactory;
use ACSB\Vendor\Humbug\PhpScoper\PhpParser\Parser\ParserFactory;
use ACSB\Vendor\Humbug\PhpScoper\PhpParser\Parser\StandardParserFactory;
use ACSB\Vendor\Humbug\PhpScoper\PhpParser\Printer\Printer;
use ACSB\Vendor\Humbug\PhpScoper\PhpParser\Printer\PrinterFactory;
use ACSB\Vendor\Humbug\PhpScoper\PhpParser\Printer\StandardPrinter;
use ACSB\Vendor\Humbug\PhpScoper\PhpParser\Printer\StandardPrinterFactory;
use ACSB\Vendor\Humbug\PhpScoper\Scoper\Factory\ScoperFactory;
use ACSB\Vendor\Humbug\PhpScoper\Scoper\Factory\StandardScoperFactory;
use ACSB\Vendor\Humbug\PhpScoper\Symbol\EnrichedReflectorFactory;
use ACSB\Vendor\Humbug\PhpScoper\Symbol\Reflector;
...
final class Container
{
    ...
    public function getScoperFactory(): ScoperFactory
    {
        if (!isset($this->scoperFactory)) {
            $this->scoperFactory = new StandardScoperFactory($this->getEnrichedReflectorFactory(), $this->getParserFactory(), $this->getPrinterFactory());
        }
    ...
    public function getPrinter(?PhpVersion $phpVersion = null): Printer
    {
        if (!isset($this->printer)) {
            ...
            $this->printer = new StandardPrinter(new Standard(['phpVersion' => $phpVersion]));
        }
    ...
    private static function checkSamePhpVersion(?PhpVersion $versionUsed, ?PhpVersion $versionRequest): void
    {
