...
return static function (?string $stubsDir = null) use ($defaultSource): array {
    ...
    $ignoredDirectories = [$packageDir . '/tests', $packageDir . '/meta'];
    $files = [];
    $collectFiles = static function (\RecursiveIteratorIterator $iterator) use (&$files, $ignoredDirectories): void {
        foreach ($iterator as $fileInfo) {
            /** @var SplFileInfo $fileInfo */
            if (\str_starts_with($fileInfo->getFilename(), '.') || $fileInfo->isDir() || !$fileInfo->isReadable() || 'php' !== $fileInfo->getExtension() || $fileInfo->getFilename() === 'PhpStormStubsMap.php') {
                continue;
            }
