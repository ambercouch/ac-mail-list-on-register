...
use ACSB\Vendor\StubTests\Model\PhpVersions;
use ACSB\Vendor\StubTests\TestData\Providers\PhpStormStubsSingleton;
use ACSB\Vendor\StubTests\TestData\Providers\Reflection\ReflectionClassesTestDataProviders;
...
class BaseClassesTest extends AbstractBaseStubsTestCase
{
    ...
    public function testClassesParent(?string $classId)
    {
        ...
        $reflectionClass = ReflectionStubsSingleton::getReflectionStubs()->getClass($classId, fromReflection: \true);
        $stubClass = PhpStormStubsSingleton::getPhpStormStubs()->getClass($classId);
        static::assertEquals($reflectionClass->parentClass, $stubClass->parentClass, empty($reflectionClass->parentClass) ? "Class {$classId} should not extend {$stubClass->parentClass}" : "Class {$classId} should extend {$reflectionClass->parentClass}");
    }
    ...
    public function testClassesFinalMethods(?string $classId, ?string $methodName)
    {
        ...
        static::assertEquals($reflectionMethod->isFinal, $stubMethod->isFinal, "Method {$classId}::{$methodName} final modifier is incorrect");
    }
    ...
    public function testClassesStaticMethods(?string $classId, ?string $methodName)
    {
        ...
        $reflectionMethod = ReflectionStubsSingleton::getReflectionStubs()->getClass($classId, fromReflection: \true)->getMethod($methodName, fromReflection: \true);
        $stubMethod = PhpStormStubsSingleton::getPhpStormStubs()->getClass($classId)->getMethod($methodName);
        static::assertEquals($reflectionMethod->isStatic, $stubMethod->isStatic, "Method {$classId}::{$methodName} static modifier is incorrect");
    }
    ...
    public function testClassesMethodsVisibility(?string $classId, ?string $methodName)
    {
        ...
        $reflectionMethod = ReflectionStubsSingleton::getReflectionStubs()->getClass($classId, fromReflection: \true)->getMethod($methodName, fromReflection: \true);
        $stubMethod = PhpStormStubsSingleton::getPhpStormStubs()->getClass($classId)->getMethod($methodName);
        static::assertEquals($reflectionMethod->access, $stubMethod->access, "Method {$classId}::{$methodName} access modifier is incorrect");
    }
    ...
    public function testClassMethodsParametersCount(?string $classId, ?string $methodName)
    {
        ...
        $reflectionMethod = ReflectionStubsSingleton::getReflectionStubs()->getClass($classId, fromReflection: \true)->getMethod($methodName, fromReflection: \true);
        $stubMethod = PhpStormStubsSingleton::getPhpStormStubs()->getClass($classId)->getMethod($methodName);
        $filteredStubParameters = array_filter($stubMethod->parameters, function ($parameter) {
            if (!empty($parameter->availableVersionsRangeFromAttribute)) {
                return $parameter->availableVersionsRangeFromAttribute['from'] <= doubleval(getenv('PHP_VERSION') ?? PhpVersions::getFirst()) && $parameter->availableVersionsRangeFromAttribute['to'] >= (doubleval(getenv('PHP_VERSION')) ?? PhpVersions::getLatest());
            } else {
                return \true;
            }
        });
        static::assertSameSize($reflectionMethod->parameters, $filteredStubParameters, "Parameter number mismatch for method {$classId}::{$methodName}.\n                         Expected: " . self::getParameterRepresentation($reflectionMethod));
    }
    ...
    public function testClassInterfaces(?string $classId)
    {
        ...
        foreach ($reflectionClass->interfaces as $interface) {
            static::assertContains($interface, $stubClass->interfaces, "Class {$classId} doesn't implement interface {$interface}");
        }
    }
    ...
    public function testClassProperties(?string $classId, ?string $propertyName)
    {
        ...
        $reflectionClass = ReflectionStubsSingleton::getReflectionStubs()->getClass($classId, fromReflection: \true);
        $reflectionProperty = $reflectionClass->getProperty($propertyName, fromReflection: \true);
        $stubClass = PhpStormStubsSingleton::getPhpStormStubs()->getClass($classId);
        static::assertNotEmpty($stubClass->getProperty($propertyName), "Missing property {$reflectionProperty->access} " . implode('|', $reflectionProperty->typesFromSignature) . "{$classId}::\${$propertyName}");
    }
    ...
    public function testClassStaticProperties(?string $classId, ?string $propertyName)
    {
        ...
        static::assertEquals($reflectionProperty->is_static, $stubProperty->is_static, "Property {$classId}::{$propertyName} static modifier is incorrect");
    }
    ...
    public function testClassPropertiesVisibility(?string $classId, ?string $propertyName)
    {
        ...
        $reflectionProperty = $reflectionClass->getProperty($propertyName, fromReflection: \true);
        $stubProperty = PhpStormStubsSingleton::getPhpStormStubs()->getClass($classId)->getProperty($propertyName);
        static::assertEquals($reflectionProperty->access, $stubProperty->access, "Property {$classId}::{$propertyName} access modifier is incorrect");
    }
    ...
    public function testClassPropertiesType(?string $classId, ?string $propertyName)
    {
        ...
        if (!$testCondition) {
            if (!empty($unifiedStubsAttributesPropertyTypes)) {
                $typesFromAttribute = !empty($unifiedStubsAttributesPropertyTypes[getenv('PHP_VERSION')]) ? $unifiedStubsAttributesPropertyTypes[getenv('PHP_VERSION')] : $unifiedStubsAttributesPropertyTypes['default'];
                $testCondition = self::isReflectionTypesExistInAttributes($unifiedReflectionPropertyTypes, $typesFromAttribute);
            }
        }
        self::assertTrue($testCondition, "Property {$classId}::{$propertyName} has invalid typehint.\n        Reflection property has type " . implode('|', $unifiedReflectionPropertyTypes) . ' but stubs has type ' . implode('|', $unifiedStubsPropertyTypes) . ' in signature and attribute has types ' . implode('|', $typesFromAttribute));
    }
    ...
    public function testClassesReadonly(?string $classId): void
    {
        ...
        $reflectionClass = ReflectionStubsSingleton::getReflectionStubs()->getClass($classId, fromReflection: \true);
        $stubClass = PhpStormStubsSingleton::getPhpStormStubs()->getClass($classId);
        static::assertEquals($reflectionClass->isReadonly, $stubClass->isReadonly, "Readonly modifier for class {$classId} is incorrect");
    }
    ...
    public function testClassesNamespace(?string $classId): void
    {
        ...
        $reflectionClass = ReflectionStubsSingleton::getReflectionStubs()->getClass($classId, fromReflection: \true);
        $stubClass = PhpStormStubsSingleton::getPhpStormStubs()->getClass($classId);
        static::assertEquals($reflectionClass->namespace, $stubClass->namespace, "Namespace for class {$classId} is incorrect");
    }
}
