...
use ACSB\Vendor\StubTests\Parsers\ExpectedFunctionArgumentsInfo;
use ACSB\Vendor\StubTests\Parsers\MetaExpectedArgumentsCollector;
...
class StubsMetaExpectedArgumentsTest extends AbstractBaseStubsTestCase
{
    ...
    {
        return self::flatten(array_map(fn(PHPClass $class) => array_map(fn(PHPMethod $method) => self::getClassMemberFqn($class->id, $method->name), $class->methods), $stubs->getClasses()));
    }
    public function testFunctionReferencesExists()
    {
        foreach (self::$expectedArguments as $argument) {
            ...
            if ($expr instanceof FuncCall) {
                ...
                if (!str_starts_with($fqn, self::PSR_LOG_LOGGER_NAMESPACE_PREFIX) && !str_starts_with($fqn, self::GUZZLE_HTTP_NAMESPACE_PREFIX) && !str_starts_with($fqn, self::ILLUMINATE_NAMESPACE_PREFIX)) {
        ...
        foreach (self::$expectedArguments as $argument) {
            ...
            if ($expr !== null) {
                ...
                if (method_exists($originalName, 'isFullyQualified')) {
                    self::assertTrue($originalName->isFullyQualified(), self::getFqn($expr) . ' should be fully qualified');
                } else {
