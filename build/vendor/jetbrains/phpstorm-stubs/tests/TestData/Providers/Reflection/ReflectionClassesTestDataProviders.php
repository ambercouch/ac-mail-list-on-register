...
class ReflectionClassesTestDataProviders
{
    ...
    {
        $filtered = EntitiesFilter::getFiltered(ReflectionStubsSingleton::getReflectionStubs()->getClasses(), fn(PHPClass $class) => empty($class->interfaces) || $class->name == 'DOMException', StubProblemType::WRONG_INTERFACE);
        if (empty($filtered)) {
            yield [null];
        } else {
            foreach ($filtered as $class) {
    ...
    {
        $filtered = EntitiesFilter::getFiltered(ReflectionStubsSingleton::getReflectionStubs()->getEnums(), fn(PHPEnum $enum) => empty($enum->interfaces), StubProblemType::WRONG_INTERFACE);
        if (empty($filtered)) {
            yield [null];
        } else {
            foreach ($filtered as $enum) {
    ...
    {
        $classes = ReflectionStubsSingleton::getReflectionStubs()->getClasses();
        $filtered = EntitiesFilter::getFiltered($classes, fn(PHPClass $class) => empty($class->parentClass) || $class->name == 'DOMException', StubProblemType::WRONG_PARENT);
        if (empty($filtered)) {
            yield [null];
        } else {
            foreach ($filtered as $class) {
    ...
    {
        $interfaces = ReflectionStubsSingleton::getReflectionStubs()->getInterfaces();
        $filtered = EntitiesFilter::getFiltered($interfaces, fn(PHPInterface $interface) => empty($interface->parentInterfaces), StubProblemType::WRONG_PARENT);
        if (empty($filtered)) {
            yield [null];
        } else {
            foreach ($filtered as $interface) {
    ...
    {
        $classes = ReflectionStubsSingleton::getReflectionStubs()->getClasses();
        $filtered = EntitiesFilter::getFiltered($classes, fn(PHPClass $class) => $class->name === "DOMException" || $class->name === '__PHP_Incomplete_Class', StubProblemType::WRONG_FINAL_MODIFIER);
        if (empty($filtered)) {
            yield [null];
        } else {
            foreach ($filtered as $class) {
