...
use ACSB\Vendor\PhpParser\Node\Expr\Array_;
use ACSB\Vendor\PhpParser\Node\Expr\BinaryOp\BitwiseOr;
use ACSB\Vendor\PhpParser\Node\Expr\ClassConstFetch;
use ACSB\Vendor\PhpParser\Node\Expr\ConstFetch;
use ACSB\Vendor\PhpParser\Node\Expr\UnaryMinus;
use ACSB\Vendor\PhpParser\Node\Scalar\DNumber;
use ACSB\Vendor\PhpParser\Node\Scalar\LNumber;
use ACSB\Vendor\PhpParser\Node\Scalar\String_;
use ACSB\Vendor\PHPUnit\Framework\Exception;
use ACSB\Vendor\PHPUnit\Framework\TestCase;
...
abstract class AbstractBaseStubsTestCase extends TestCase
{
    ...
    public static function getStringRepresentationOfDefaultParameterValue(mixed $defaultValue, PHPClass|PHPInterface|null $contextClass = null): float|bool|int|string|null
    {
        if ($defaultValue instanceof ConstFetch) {
            ...
            if ($defaultValue->left instanceof ConstFetch && $defaultValue->right instanceof ConstFetch) {
                $constants = array_filter(PhpStormStubsSingleton::getPhpStormStubs()->getConstants(), fn($const) => property_exists($defaultValue->left, 'name') && $const->name === (string) $defaultValue->left->name);
                /** @var PHPConstant $leftConstant */
                $leftConstant = array_pop($constants);
                $constants = array_filter(PhpStormStubsSingleton::getPhpStormStubs()->getConstants(), fn($const) => property_exists($defaultValue->right, 'name') && $const->name === (string) $defaultValue->right->name);
            ...
            $parentClass = PhpStormStubsSingleton::getPhpStormStubs()->getEnum($class) ?? PhpStormStubsSingleton::getPhpStormStubs()->getClass($class) ?? PhpStormStubsSingleton::getPhpStormStubs()->getInterface($class);
            if ($parentClass === null) {
    ...
    protected static function getDuplicatedFunctions(array $filtered): array
    {
        $duplicatedFunctions = array_filter($filtered, function (PHPFunction $value, int|string $key) {
            ...
            $functionVersions[] = ParserUtils::getAvailableInVersions(PhpStormStubsSingleton::getPhpStormStubs()->getFunction($value->name, shouldSuitCurrentPhpVersion: \false));
            array_push($functionVersions, ...array_values(array_map(fn(PHPFunction $function) => ParserUtils::getAvailableInVersions($function), $duplicatesOfFunction)));
            $hasDuplicates = \false;
    ...
    {
        return array_filter(PhpStormStubsSingleton::getPhpStormStubs()->getFunctions(), fn($duplicateValue, $duplicateKey) => $duplicateValue->name === $name && str_contains($duplicateKey, 'duplicated'), \ARRAY_FILTER_USE_BOTH);
    }
    /**
    ...
    {
        return empty(array_merge(array_diff($reflectionTypes, $typesFromSignature), array_diff($typesFromSignature, $reflectionTypes)));
    }
    /**
     * @param string[] $reflectionTypes
    ...
    {
        return empty(array_merge(array_diff($reflectionTypes, array_filter($typesFromAttribute, fn($type) => !empty($type))), array_diff(array_filter($typesFromAttribute, fn($type) => !empty($type)), $reflectionTypes)));
    }
    /**
     * @param string[][] $typesFromAttribute
     */
    ...
    public function getClassLikeFromStubs(string $classId, bool $shouldSuiteCurrentPHPVersion = \true): PHPInterface|PHPEnum|PHPClass|null
    {
