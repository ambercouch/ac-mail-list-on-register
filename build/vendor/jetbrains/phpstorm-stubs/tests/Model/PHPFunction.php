...
use ACSB\Vendor\phpDocumentor\Reflection\DocBlock\Tags\Param;
use ACSB\Vendor\phpDocumentor\Reflection\DocBlock\Tags\Return_;
use ACSB\Vendor\phpDocumentor\Reflection\PseudoTypes\List_;
use ACSB\Vendor\phpDocumentor\Reflection\Type;
use ACSB\Vendor\phpDocumentor\Reflection\Types\Array_;
use ACSB\Vendor\phpDocumentor\Reflection\Types\Collection;
use ACSB\Vendor\phpDocumentor\Reflection\Types\Compound;
use ACSB\Vendor\PhpParser\Comment\Doc;
use ACSB\Vendor\PhpParser\Node\FunctionLike;
use ACSB\Vendor\PhpParser\Node\Stmt\Function_;
use ReflectionFunction;
use ReflectionFunctionAbstract;
use RuntimeException;
use stdClass;
use ACSB\Vendor\StubTests\Parsers\DocFactoryProvider;
...
class PHPFunction extends PHPNamespacedElement
{
    ...
    public function getParameter(string $parameterName)
    {
        $parameters = array_filter($this->parameters, function (PHPParameter $parameter) use ($parameterName) {
            return $parameter->name === $parameterName && $parameter->duplicateOtherElement === \false && ParserUtils::entitySuitsCurrentPhpVersion($parameter);
        });
        if (empty($parameters)) {
