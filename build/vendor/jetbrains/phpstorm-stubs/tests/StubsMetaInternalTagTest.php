...
use ACSB\Vendor\StubTests\Parsers\Visitors\MetaOverrideFunctionsParser;
...
class StubsMetaInternalTagTest extends AbstractBaseStubsTestCase
{
    ...
    public function testFunctionInternalMetaTag(): void
    {
        ...
        foreach ($functions as $function) {
            if ($function->hasInternalMetaTag) {
                $reflectionFunctions = array_filter(ReflectionStubsSingleton::getReflectionStubs()->getFunctions(), fn($refFunction) => $refFunction->name === $function->name);
        ...
        foreach (PhpStormStubsSingleton::getPhpStormStubs()->getClasses() as $className => $class) {
            foreach ($class->methods as $methodName => $method) {
                if ($method->hasInternalMetaTag) {
                    $refClass = ReflectionStubsSingleton::getReflectionStubs()->getClass($className, fromReflection: \true);
                    if ($refClass !== null) {
                        $reflectionMethods = array_filter($refClass->methods, fn($refMethod) => $refMethod->name === $methodName);
    ...
    {
        self::assertContains($elementName, self::$overriddenFunctionsInMeta, "{$elementName} contains @meta in phpdoc but isn't added to 'override()' functions in meta file");
    }
}
