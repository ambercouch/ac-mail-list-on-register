...
class StubsParametersProvider
{
    ...
    private static function yieldFilteredMethodParameters(string $classType, callable $filterFunction, int ...$problemTypes): ?Generator
    {
        ...
        $toYield = array_filter(array_map(fn($class) => array_filter(array_map(fn(PHPMethod $method) => array_filter(EntitiesFilter::getFilteredParameters($method, null, ...$problemTypes), function ($parameter) use ($filterFunction, $class, $method) {
        ...
        }), EntitiesFilter::getFilteredStubsMethods($class)), fn($parameters) => !empty($parameters)), $filtered), fn($methods) => !empty($methods));
        if (empty($toYield)) {
    ...
    private static function getFilterFunctionForLanguageLevel(string $classType, float $languageVersion): callable
    {
        return match ($classType) {
            PHPClass::class => fn(PHPClass $class, PHPMethod $method, ?float $firstSinceVersion) => !$method->isFinal && !$class->isFinal && $firstSinceVersion !== null && $firstSinceVersion < $languageVersion,
            PHPInterface::class => fn(PHPInterface $class, PHPMethod $method, ?float $firstSinceVersion) => !$method->isFinal && !$class->isFinal && $firstSinceVersion !== null && $firstSinceVersion < $languageVersion,
            PHPEnum::class => fn(PHPEnum $class, PHPMethod $method, ?float $firstSinceVersion) => !$method->isFinal && !$class->isFinal && $firstSinceVersion !== null && $firstSinceVersion < $languageVersion,
