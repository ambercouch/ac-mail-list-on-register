...
class StubsParameterNamesTest extends AbstractBaseStubsTestCase
{
    ...
    public function testFunctionsParameterNames(?string $functionId, ?string $parameterName)
    {
        ...
        self::assertNotEmpty(array_filter($phpstormFunction->parameters, fn(PHPParameter $stubParameter) => $stubParameter->name === $parameterName), "Function {$functionId} has signature {$functionId}(" . self::printParameters($reflectionFunction->parameters) . ')' . " but stub function has signature {$phpstormFunction->name}(" . self::printParameters($phpstormFunction->parameters) . ')');
    }
    ...
    public function testClassMethodsParameterNames(?string $classId, ?string $methodName, ?string $parameterName)
    {
        ...
        self::assertNotEmpty(array_filter($stubMethod->parameters, fn(PHPParameter $stubParameter) => $stubParameter->name === $reflectionParameter->name), "Method {$classId}::{$methodName} has signature {$methodName}(" . self::printParameters($reflectionMethod->parameters) . ')' . " but stub function has signature {$methodName}(" . self::printParameters($stubMethod->parameters) . ')');
    }
