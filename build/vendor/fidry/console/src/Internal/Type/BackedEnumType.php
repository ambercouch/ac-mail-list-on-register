...
namespace ACSB\Vendor\Fidry\Console\Internal\Type;

use BackedEnum;
use ACSB\Vendor\Fidry\Console\Input\InvalidInputValueType;
...
final class BackedEnumType implements InputType
{
     ...
     */
    public function __construct(private readonly string $backedEnumClassName)
    {
    }
    public function coerceValue(array|bool|string|null $value, string $label): BackedEnum
    {
        ...
        try {
            ...
            throw new InvalidInputValueType(sprintf('Expected a value "%s" enum. Got "%s" for %s.', $this->backedEnumClassName, InputAssert::castType($value), $label));
        }
    }
