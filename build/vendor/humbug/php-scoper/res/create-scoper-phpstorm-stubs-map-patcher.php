...
return static function (?string $stubsMapPath = null, ?string $stubsMapVendorPath = null): \Closure {
    ...
    return static function (string $filePath, string $prefix, string $contents) use ($stubsMapVendorPath, $stubsMapClassOriginalContent): string {
        if ($filePath !== $stubsMapVendorPath) {
            return $contents;
        }
        return \preg_replace('/class PhpStormStubsMap([\s\S]+)/', 'class PhpStormStubsMap' . $stubsMapClassOriginalContent, $contents);
    };
};
