...
use ACSB\Vendor\PhpParser\Node\ArrayItem;
use ACSB\Vendor\PhpParser\Node\Const_;
use ACSB\Vendor\PhpParser\Node\Expr\Array_;
use ACSB\Vendor\PhpParser\Node\Expr\Assign;
use ACSB\Vendor\PhpParser\Node\Expr\FuncCall;
use ACSB\Vendor\PhpParser\Node\Expr\New_;
use ACSB\Vendor\PhpParser\Node\Expr\StaticCall;
...
use ACSB\Vendor\PhpParser\Node\Param;
use ACSB\Vendor\PhpParser\Node\Scalar\String_;
use ACSB\Vendor\PhpParser\Node\Stmt\PropertyProperty;
use ACSB\Vendor\PhpParser\Node\Stmt\Return_;
...
final class StringScalarPrefixer extends NodeVisitorAbstract
{
    ...
    private const SPECIAL_FUNCTION_NAMES = ['call_user_func_array', 'class_alias', 'class_exists', 'define', 'defined', 'function_exists', 'interface_exists', 'is_a', 'is_callable', 'is_subclass_of', 'method_exists', 'spl_autoload_register', 'trait_exists'];
    // Function for which we know the FIRST argument IS a FQCN
    private const SPECIAL_ARRAY_FUNCTION_NAMES = ['call_user_func_array', 'is_callable', 'spl_autoload_register'];
...
$/ux
REGEX;
    public function __construct(private readonly string $prefix, private readonly EnrichedReflector $enrichedReflector, private readonly ExcludedFunctionExistsStringNodeStack $excludedFunctionExistsStringNodeStack)
    {
    }
    ...
    {
        $class = $newNode->class;
        if ($class instanceof Name && in_array(strtolower($class->toString()), self::DATETIME_CLASSES, \true)) {
    ...
    private function prefixFunctionStringArg(String_ $string, FuncCall $functionNode, string $normalizedValue): String_
    {
        ...
        if ('method_exists' === $functionName) {
            ...
            $isFirstArgument = $firstArgument instanceof Arg && $firstArgument->value === $string;
            if ($isFirstArgument) {
                return $this->enrichedReflector->isClassExcluded($normalizedValue) ? $string : $this->createPrefixedString($string);
            }
            return $string;
        }
        return $this->enrichedReflector->isClassExcluded($normalizedValue) || $this->enrichedReflector->isFunctionExcluded($normalizedValue) ? $string : $this->createPrefixedString($string);
    }
    ...
    private function prefixArrayItemString(String_ $string, ArrayItem $parentNode, string $normalizedValue): String_
    {
        ...
        return in_array($functionName, self::SPECIAL_ARRAY_FUNCTION_NAMES, \true) && array_key_exists(0, $arrayNode->items) && $arrayItemNode === $arrayNode->items[0] && !$this->enrichedReflector->isClassExcluded($normalizedValue) ? $this->createPrefixedString($string) : $string;
    }
