...
namespace ACSB\Vendor\GuzzleHttp\Handler;

use ACSB\Vendor\GuzzleHttp\Exception\ConnectException;
use ACSB\Vendor\GuzzleHttp\Exception\RequestException;
use ACSB\Vendor\GuzzleHttp\Promise as P;
use ACSB\Vendor\GuzzleHttp\Promise\FulfilledPromise;
use ACSB\Vendor\GuzzleHttp\Promise\PromiseInterface;
use ACSB\Vendor\GuzzleHttp\Psr7\LazyOpenStream;
use ACSB\Vendor\GuzzleHttp\TransferStats;
use ACSB\Vendor\GuzzleHttp\Utils;
use ACSB\Vendor\Psr\Http\Message\RequestInterface;
use ACSB\Vendor\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \ACSB\Vendor\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \ACSB\Vendor\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \ACSB\Vendor\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \ACSB\Vendor\GuzzleHttp\Psr7\Utils::streamFor($sink);
